...
const handleRequest = async (
  ...
  const method = request.method?.toLowerCase() as MethodType;
  const endpoints: Route = routes[method] || {};
  const url = removeLastSlash(request.url);
  const [route, callback] =
    Object.entries(endpoints).find(
      ([endpoint]) => url && isMatchEndpoint(endpoint, url)
    ) || [];
  ...
  if (route && callback) {
    try {
      const { userId } = getEndpointIds(route, url);
...
export const requestListener: RequestListener = handleRequest;
